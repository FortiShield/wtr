...
This document provides context for GitHub Copilot regarding the architecture and execution flow of the `branchops` tool.

## Core Architecture
- **Language**: Pure Bash (targeting 4.0+).
- **Entry Point**: `bin/branchops` is the main executable script.
- **Library System**: `bin/branchops` sources modular logic from the `lib/` directory.
  - `lib/log.sh`: ANSI-colored logging (success, info, warn, error, step).
  - `lib/config.sh`: Wrappers for `git config` and file-based `.branchopsconfig`.
...
## Command Dispatcher
The `main()` function in `bin/branchops` uses a `case` statement to dispatch commands to their respective `cmd_` functions (e.g., `create` -> `cmd_create`).
...
3. **Preset Loading**: Applies defaults from `branchops.preset.<name>`.
4. **Pre-Create Hook**: Executes `preCreate` scripts from `.branchops/hooks/`.
...
- **Config Precedence**: CLI Flags > Local Config (`.git/config`) > Project Config (`.branchopsconfig`) > Global Config.
